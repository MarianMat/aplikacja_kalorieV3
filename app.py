...
if option == "Ręcznie":
    sub_option = st.radio("Jak chcesz dodać posiłek?", ["✍️ Wpisz dane samodzielnie", "🧠 Opisz posiłek (AI)"])

    # ✍️ Ręczne wpisywanie
    if sub_option == "✍️ Wpisz dane samodzielnie":
        add_meal_form(st.session_state.username)

    # 🧠 AI analiza opisu
    elif sub_option == "🧠 Opisz posiłek (AI)":
        description = st.text_area("📝 Opisz posiłek (np. 'miska płatków z mlekiem i bananem')")
        if st.button("🤖 Analizuj opis posiłku"):
            if description.strip():
                result = estimate_calories_from_image(description)
                if isinstance(result, dict):
                    st.success("✅ AI przeanalizowało opis!")
                    st.json(result)
                    st.session_state.prefill = result
                    add_meal_form(st.session_state.username)
                else:
                    st.error("❌ Nie udało się uzyskać danych z AI.")
            else:
                st.warning("⚠️ Podaj opis posiłku.")
